...
SELECT_POLL_AND_VOTE_DETAILS = """SELECT options.id, options.option_text,
...
def get_poll_details(connection, poll_id: int) -> List[PollWithOption]: # Type hinting Example
    with connection:
        with connection.cursor() as cursor:
...
def get_poll_and_vote_results(connection, poll_id: int) -> List[PollResults:
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(SELECT_POLL_AND_VOTE_DETAILS, (poll_id,))
            return cursor.fetchall()
